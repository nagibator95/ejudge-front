...
ConfigParser -- responsible for parsing a list of
    ...
    remove_option(section, idx, option)
        Remove the given option from the given section.

    set(section, idx, option, value)
...
class Interpolation:
    ...
    def before_get(self, parser, section, idx, option, value, defaults):
        return value

    def before_set(self, parser, section, option, value):
        return value

    def before_read(self, parser, section, idx, option, value):
        return value
...
class BasicInterpolation(Interpolation):
    ...
    def before_get(self, parser, section, idx, option, value, defaults):
        L = []
#        print(value)
        self._interpolate_some(parser, option, L, value, section, idx, defaults, 1)
        return ''.join(L)
    ...
    def _interpolate_some(self, parser, option, accum, rest, section, idx, map,
                          depth):
...
class RawConfigParser(MutableMapping):
    ...
    def __init__(self, defaults=None, dict_type=_default_dict,
        ...
        self._proxies[default_section] = UserList([ SectionProxy(self, default_section, 0) ])
        if defaults:
...
#            raise DuplicateSectionError(section)
        ...
        self._proxies[section].append(SectionProxy(self, section, len(self._sections[section]) - 1))
        return res
...
#        print(option)
        try:
            d = self._unify_values(section, idx, vars)
...
#        print(d)
        try:
            ...
            if fallback is _UNSET:
                raise NoOptionError(option, section[idx])
            else:
                return fallback
        ...
        if raw or value is None:
            ...
            return self._interpolation.before_get(self, section, idx, option, value,
                                                  d)
    ...
    def items(self, section=_UNSET, idx = 0, raw=False, vars=None):
...
#        sectdict[self.optionxform(option)] = value
    ...
    def remove_option(self, section, idx, option):
        """Remove an option."""
        if not section or section == self.default_section:
            sectdict = self._defaults
        else:
...
#                        print(111)
                        ...
                        self._proxies[sectname].append(SectionProxy(self, sectname, 0))
                        elements_added.add(sectname)
...
#        self._join_multiline_values()
    ...
    def _unify_values(self, section, idx, vars):
        ...
        sectiondict = {}
        try:
            sectiondict = self._sections[section][idx]
#            print(sectiondict)
        except KeyError:
            if section != self.default_section:
                raise NoSectionError(section)
...
class SectionProxy(MutableMapping):
    ...
    def __init__(self, parser, name, idx):
    ...
    def __repr__(self):
        return '<Section: {} {}>'.format(self._name, self._idx)

    def __getitem__(self, key):
        if not self._parser.has_option(self._name, self._idx, key):
            raise KeyError(key)
#        print(key)
        return self._parser.get(self._name, self._idx, key)

    def __setitem__(self, key, value):
        self._parser._validate_value_types(option=key, value=value)
        return self._parser.set(self._name, self._idx, key, value)

    def __delitem__(self, key):
        if not (self._parser.has_option(self._name, self._idx, key) and
                self._parser.remove_option(self._name, self._idx, key)):
            raise KeyError(key)

    def __contains__(self, key):
        return self._parser.has_option(self._name, self._idx, key)
    ...
    def _options(self):
        if self._name != self._parser.default_section:
            return self._parser.options(self._name, self._idx)
        else:
            return self._parser.defaults()

    def get(self, option, fallback=None, raw=False, vars=None):
        return self._parser.get(self._name, self._idx, option, raw=raw, vars=vars,
                                fallback=fallback)

    def getint(self, option, fallback=None, raw=False, vars=None):
        return self._parser.getint(self._name, self._idx, option, raw=raw, vars=vars,
                                   fallback=fallback)

    def getfloat(self, option, fallback=None, raw=False, vars=None):
        return self._parser.getfloat(self._name, self._idx, option, raw=raw, vars=vars,
                                     fallback=fallback)

    def getboolean(self, option, fallback=None, raw=False, vars=None):
        return self._parser.getboolean(self._name, self._idx, option, raw=raw, vars=vars,
                                       fallback=fallback)
...
class _ChainMap(DictMixin):
    ...
    def __setitem__(self, key, valuse):
        raise "NotImpl"
